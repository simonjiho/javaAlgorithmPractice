...
public class Boj_17825 {
	...
	public static void main(String args[]) throws Exception {
		...
		Node startNode = new Node(0); // 시작
		Node endNode = new Node(0); // 도착
	...
	static void dfs(int cnt, int sum, Node[] nodes) {
		...
		for(int i = 0; i < 4; i++) {
			...
			curNode.pieceOn = true;
			
			if (node.next_blue != null) {
				int move = dice[cnt];
				Node curNode = node;
				curNode = node.next_blue;
							
				for(int i = 0; i < move-1; i++) {
					curNode = curNode.next;
				}
				
				dfs(cnt+1, sum+curNode.score, curNode);
			}
			...
			curNodes[i] = curNode;
			dfs(cnt+1, sum+curNode.score, curNodes);
			
			curNode.pieceOn = false;
